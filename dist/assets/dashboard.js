...
function renderData(){document.querySelectorAll('[data-event-count]').forEach(item=>item.textContent=String(state.events.length));renderUpcoming();prepareYearSelectors();renderCalendars();renderEventManagement();renderPartners();if(document.querySelector('[data-registration-list]'))renderRegistrationManager()}
