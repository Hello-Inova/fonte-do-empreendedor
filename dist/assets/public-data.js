...
function partnerContactActions(partner){
  ...
  const action=(href,label)=>href?`<a href="${publicEscape(href)}" target="_blank" rel="noopener noreferrer" aria-label="Abrir ${label} de ${publicEscape(partner.companyName)}" title="${label}">${icons[label]}</a>`:`<span class="is-disabled" aria-label="${label} não informado" title="${label} não informado" aria-disabled="true">${icons[label]}</span>`;
